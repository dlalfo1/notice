...
public class NoticeServiceImpl implements NoticeService {
	...
	public int modifyNotice(HttpServletRequest request) {
		...
		int noticeNo = Integer.parseInt(opt1.orElse("0"));
		Optional<String> opt2 = Optional.ofNullable(request.getParameter("gubun"));
		int gubun = Integer.parseInt(opt2.orElse("0"));
		String title = request.getParameter("title");
		String content = request.getParameter("content");
		
		 NoticeDTO notice = new NoticeDTO(noticeNo, gubun, title, content);
