	...
	<div id="edit_screen">
		...
		<form method="post" action="${contextPath}/notice/modify.do">
			<div>
				<span>구분</span>
				<select name="gubun">
					<option value="1" <c:if test="${notice.gubun == '1'}">selected</c:if>>일반</option>
 					<option value="2" <c:if test="${notice.gubun == '2'}">selected</c:if>>긴급</option>
				</select>
			</div>
			<div>
			...
			<div>
				...
				<textarea cols="30"  rows="5" id="content" name="content" >${notice.content}</textarea>
			</div>
			<div>
